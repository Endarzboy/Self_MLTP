...
class SpatioTemporalConv(nn.Module):
    ...
    Args:
        ...
        padding (int or tuple, optional): Zero-padding added to the sides of the input during their respective
        convolutions.
    Default: 0
...
class SpatioTemporalResBlock(nn.Module):
    r"""Single block for the ResNet network. Uses SpatioTemporalConv in
        ...
        Args:
            ...
            downsample (bool, optional): If ``True``, the output size is to be smaller than the input.
        Default: ``False``
        """
...
class SpatioTemporalResLayer(nn.Module):
    r"""Forms a single layer of the ResNet network, with a number of repeating blocks of same output size stacked on
    top of each other
    Args:
        in_channels (int): Number of channels in the input tensor.
        ...
        block_type (Module, optional): Type of block that is to be used to form the layer.
    Default: SpatioTemporalResBlock. downsample (bool, optional): If ``True``,
...
class R3DNet(nn.Module):
    ...
    512-dimensional vector for each element in the batch.
        ...
        Default: SpatioTemporalResBlock.
    """

    def __init__(self, layer_sizes, block_type=SpatioTemporalResBlock, num_classes=3):
        super(R3DNet, self).__init__()
       
        # first conv, with stride 1x2x2 and kernel size 1x7x7
        self.conv1 = SpatioTemporalConv(1, 64, [3, 7, 7], stride=[1, 2, 2], padding=[1, 3, 3]) # uncomment this while pre-training
...
#     r3d = R3DNet((2, 2, 2, 2), num_classes=3).cuda()
#     input_ = torch.randn((8, 1, 8, 112, 112)).cuda()
#     output = r3d(input_)
#     print("output: ", output.shape)
#     summary(r3d, (1, 16, 112, 112))
